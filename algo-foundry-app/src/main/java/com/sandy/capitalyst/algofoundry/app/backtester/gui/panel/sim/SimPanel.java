package com.sandy.capitalyst.algofoundry.app.backtester.gui.panel.sim;
...
import com.sandy.capitalyst.algofoundry.app.backtester.gui.indchart.*;
import com.sandy.capitalyst.algofoundry.app.backtester.gui.indchart.util.CrossHairMoveListener;
...
public class SimPanel extends JPanel {
    ...
    public SimPanel( String symbol ) throws Exception {
        ...
        populateStrategyConfigOverrideValues( this.config ) ;
    ...
    private void populateStrategyConfigOverrideValues( MyStrategyConfig cfg )
            throws Exception {
        
        InputStream is = SimPanel.class.getResourceAsStream( "/strategy-parameters-override.properties" ) ;
        if( is != null ) {
            ...
            prop.forEach( (k,v) -> {
                try {
                    BeanUtils.setProperty( ( Object )cfg, ( String )k, Float.parseFloat( (String)v ) ) ;
                    log.debug( "Overriding strategy config {} = {}", k, v ) ;
                }
                catch( Exception e ) {
    ...
    public void simulationEnded() { log.debug( "\n" + tradeBook.toString() ) ; }
